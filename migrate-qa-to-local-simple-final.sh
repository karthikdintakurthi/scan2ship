...
# Simple Final QA to Local Database Migration Script
# This script migrates all data from QA database to local PostgreSQL database
# Uses the existing Prisma schema and just copies data

set -e  # Exit on any error

echo "🚀 Starting Simple Final QA to Local Database Migration..."
...
# Now copy data from QA to local using simple INSERT statements
...
for table in $TABLES; do
    if [ ! -z "$table" ]; then
        ...
        if [ "$QA_COUNT" -gt 0 ]; then
            # Copy data using a simple approach - export to CSV and import
            echo "📤 Exporting data from QA table: $table"
            
            # Handle special cases for problematic tables
            if [ "$table" = "client_order_configs" ]; then
                echo "🔧 Using custom export for client_order_configs table..."
                # Export with data type conversion using a different approach
                ...
                FROM \"$table\"" | tail -n +2 | sed 's/|/,/g' > "$BACKUP_DIR/qa_${table}_data.csv"
            else
