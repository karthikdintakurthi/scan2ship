...
async function fixQAUserAuth() {
  ...
  try {
    ...
    const existingUsers = await qaPrisma.user.findMany({
      select: {
        id: true,
        email: true,
        name: true,
    ...
    const karthikUser = await qaPrisma.user.findFirst({
    ...
    if (karthikUser) {
      ...
      const newUser = await qaPrisma.user.create({
        data: {
    ...
    // Test login
    console.log('\n3. Testing login...');
    const testUser = await qaPrisma.user.findFirst({
      where: { email: 'karthik@scan2ship.in' }
    });
